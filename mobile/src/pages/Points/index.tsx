...
const Points: React.FC = () => {
  ...
  React.useEffect(() => {
    async function loadPosition(): Promise<void> {
      ...
      if (status !== 'granted') {
        Alert.alert('Sem sua permissão não conseguimos prosseguir.');
        return;
      }
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <View style={styles.itemsContainer}>
        ...
        >
          {items.map(({ id, image_url: imageUrl, title }) => (
            <TouchableOpacity
              key={String(id)}
              style={[
                styles.item,
                selectedItems.includes(id) ? styles.selectedItem : null,
              ]}
              activeOpacity={0.6}
              onPress={() => {
                handleSelectItem(id);
            ...
            >
              <SvgUri width={42} height={42} uri={imageUrl} />
